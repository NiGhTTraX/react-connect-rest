...
import RestStore2 from './components/rest-store2';
import { Omit } from 'react-bind-component';
import FetchTransport from './lib/fetch-transport';

export function connectToRest2<
  ViewProps extends Record<K, RestStore2<T>>,
  T,
  K extends string
>(
 ...
 * @param prop
 */
export default function connectToRest2<
  ViewProps extends Record<K, RestStore2<T>>,
  T,
  K extends string
>(
  View: ComponentType<ViewProps>,
  containerOrApi: RestStore2<T> | string,
  prop: K
): ComponentType<Omit<ViewProps, K>> {
  if (typeof containerOrApi === 'string') {
    return connectToState(View, new RestStore2(FetchTransport, containerOrApi), prop);
  }
